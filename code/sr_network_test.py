...
with tf.device(CONST.SEL_GPU) :
	...
	class SrNet () :
		def infer (self, n, short_cut, sizeImage ):
			...
			for i in xrange(n) :
				...
				if i == 0 :
					self.gr_mat1[i] = inst_res_unit(self.relu_intro, i, sizeFeature, 64, short_cut, 1, 1 )
				else :
					self.gr_mat1[i] = inst_res_unit(self.gr_mat1[i-1].out, i, sizeFeature, 64, short_cut, 1, 0 )
	...
	class inst_res_unit(object):
		if CONST.BOTTLENECK == 1 :
			def __init__(self, input_x, index, sizeFeature, filt_depth, short_cut, stride, IsFirst):
