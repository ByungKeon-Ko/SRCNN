...
with tf.device(CONST.SEL_GPU) :
	...
	class SrNet () :
		def infer (self, n, short_cut ):
			...
			self.linear_intro	= conv2d(self.x, self.W_conv_intro, 1) + self.B_conv_intro
			...
			for i in xrange(n) :
				if i == 0 :
					self.gr_mat1[i] = inst_res_unit(self.relu_intro, i, CONST.lenPATCH-2-2*i, 64, short_cut, 1, 1 )
				else :
					self.gr_mat1[i] = inst_res_unit(self.gr_mat1[i-1].out, i, CONST.lenPATCH-2-2*i, 64, short_cut, 1, 0 )
			...
			self.x_center	= tf.slice(self.x, [0, 1, 1, 0], [-1, CONST.lenPATCH-2*(1+CONST.nLAYER), CONST.lenPATCH-2*(1+CONST.nLAYER), 3] )
		...
		def objective (self):
			...
			self.mse 		= tf.reduce_mean(tf.reduce_mean(tf.reduce_mean(tf.square(self.image_gen -self.y_center))))
	...
	class inst_res_unit(object):
		if CONST.BOTTLENECK == 1 :
			def __init__(self, input_x, index, map_len, filt_depth, short_cut, stride, IsFirst):
				...
				if short_cut :
					self.shortcut_path = tf.slice(input_x, [0, 1, 1, 0], [-1, map_len-2, map_len-2, 64] )
			...
			def __init__(self, input_x, index, map_len, filt_depth, short_cut, stride, IsFirst):
				...
				if short_cut :
					self.shortcut_path = tf.slice(input_x, [1, 1, 0], [map_len, map_len, 3] )
