...
with tf.device(CONST.SEL_GPU) :
	...
	class SrNet () :
		...
		## Abstract Path Version
			...
			def infer (self, n, short_cut, sizeImage, zero_pad ):
				...
				sizeFeature = [sizeImage[0], sizeImage[1]]
				if CONST.ABS_PATH == 1 : 
					self.abstr_unit = abstr_unit(self.x, sizeFeature, self.phase_train)
				...
				for i in xrange(n) :
					sizeFeature = [sizeImage[0], sizeImage[1]]
					if i == 0 :
						self.gr_mat1[i] = inst_res_unit(self.relu_intro, i, sizeFeature, 0, 1, 1, zero_pad, self.phase_train, self.abstr_path )
					else :
						self.gr_mat1[i] = inst_res_unit(self.gr_mat1[i-1].out, i, sizeFeature, 0, 1, 0, zero_pad, self.phase_train, self.abstr_path )
				...
				if short_cut :
					...
					for i in xrange(m) :
						sizeFeature = [sizeImage[0], sizeImage[1]]
						if i == 0 :
							self.gr_mat2[i] = inst_res_unit(self.gr_mat1[n-1].out, i, sizeFeature, 1, 1, 1, zero_pad, self.phase_train, self.abstr_path )
						else :
							self.gr_mat2[i] = inst_res_unit(self.gr_mat2[i-1].out, i, sizeFeature, 1, 1, 0, zero_pad, self.phase_train, self.abstr_path )
		...
		def train (self, LearningRate ):
			...
			self.capped_gvs      = [(tf.clip_by_value(grad, -1e-5, 1e-5), var) for grad, var in self.gvs]
	...
	if CONST.ABS_PATH == 1 :
		class inst_res_unit(object):
			def __init__(self, input_x, index, sizeFeature, short_cut, stride, IsFirst, zero_pad, phase_train, abstr_path):
				...
				self.linear_unit1	= conv2d(self.concat1, self.W_conv1, 1, zero_pad) + self.B_conv1
				...
				self.linear_unit2	= conv2d(self.concat2, self.W_conv2, 1, zero_pad) + self.B_conv2
	...
	class abstr_unit(object):
		def __init__(self, input_x, sizeFeature, phase_train):
			...
			## Receptive field 2x2
			self.W_conv1	= weight_variable ( [3, 3, 1, depth], 'w_conv_abs1', k2d )
