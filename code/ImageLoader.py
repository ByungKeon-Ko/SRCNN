...
def ImageLoad():
	...
	dset_train = [0]*91
	for i in xrange(91) :
		dset_train[i] = np.asarray( Image.open( "%s%s" %(path_train, datalist[i].rstrip() ) ).convert('RGB'), np.uint8)
	...
	for i in xrange( len(datalist) ) :
		...
		dset_test[i] = np.asarray( Image.open( "%s%s" %(path_test, datalist[i].rstrip() ) ).convert('RGB'), np.uint8)
